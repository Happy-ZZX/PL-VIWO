...
public:
  ...
  /**
   * @brief Given a raw uv line, this will undistort it based on the camera matrices into normalized camera coords.
   * @param uv_dist Raw uv coordinate we wish to undistort
   * @return 2d vector of normalized coordinates
   */
  Eigen::Vector4f undistort_line(const Eigen::Vector4f &line_dist) {
    Eigen::Vector2f ept1, ept2;
    Eigen::Vector2f ept3, ept4;
    ept1 << line_dist(0), line_dist(1);
    ept2 << line_dist(2), line_dist(3);
    ept3 = undistort_f(ept1);
    ept4 = undistort_f(ept2);
    Eigen::Vector4f line_out;
    line_out(0) = ept3(0);
    line_out(1) = ept3(1);
    line_out(2) = ept4(0);
    line_out(3) = ept4(1);
